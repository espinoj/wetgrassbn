function welcome(inputs){
 ...
 var cloudy = inputs.cloudy;
 var wetgrass = inputs.wetgrass;
 var sprinkler = inputs.sprikler;
 var rain = inputs.rain;

 var jsbayes = defineLib();


 var g = jsbayes.newGraph();
 var nCloudy = g.addNode('cloudy', ['t', 'f']);
 var nWetgrass = g.addNode('wetgrass', ['t', 'f']);
 var nSprinkler = g.addNode('sprinkler', ['t', 'f']);
 var nRain = g.addNode('rain', ['t', 'f']);

 nWetgrass.addParent(nRain);
 nWetgrass.addParent(nSprinkler);

 nRain.addParent(nCloudy);
 nSprinkler.addParent(nCloudy);

 nCloudy.cpt = [0.5, 0.5];
 nRain.cpt = [ [ 0.8, 0.2 ], [ 0.2, 0.8 ] ];
 nSprinkler.cpt = [ [ 0.5, 0.5 ], [ 0.1, 0.9 ] ];
 nWetgrass.cpt = [
      [  // nRain = t
          // nSprinkler = t
       [ 0.99, 0.01 ], //[ P(n3=0|n1=0,n2=0), P(n3=1|n1=0,n2=0) ]
          // nSprinkler = f
       [ 0.9, 0.1 ]  //[ P(n3=0|n1=0,n2=1), P(n3=1|n1=0,n2=1) ]
      ],
      [  // nRain = f
          // nSprinkler = t
       [ 0.9, 0.1 ], //[ P(n3=0|n1=1,n2=0), P(n3=1|n1=1,n2=0) ]
       // nSprinkler = f
       [ 0.0, 1.0 ]  //[ P(n3=0|n1=1,n2=1), P(n3=1|n1=1,n2=1) ]
      ]
     ];

 g.sample(10000);

 cloudyTrue = nCloudy.probs()[0];
 rainTrue = nRain.probs()[0];
 sprinklerTrue = nSprinkler.probs()[0];
 wetGrassTrue = nWetgrass.probs()[0];

 var returnStr = "P(Cloudy|Evidence) = " + cloudyTrue + ", ";
 returnStr = returnStr + "P(Rain|Evidence) = " + rainTrue + ", ";
 returnStr = returnStr + "P(Sprinkler|Evidence) = " + sprinklerTrue + ", ";
 returnStr = returnStr + "P(WetGrass|Evidence) = " + wetGrassTrue + ", ";
